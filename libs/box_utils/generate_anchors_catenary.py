...
def generate_anchors(base_size=16, ratios=[0.5, 1, 2],
    ...
    base_anchor = np.array([1, 1, 4, 4]) - 1   # 4 change by wu
    ratio_anchors = _ratio_enum(base_anchor, ratios)
    anchors = np.vstack([_scale_enum(ratio_anchors[i, :], scales)
                         for i in range(ratio_anchors.shape[0])])
    return anchors.astype(np.float32)
...
def _mkanchors(ws, hs, x_ctr, y_ctr):
    ...
    #                      y_ctr + 0.5 * (hs - 1)))
    #### right 
    anchors1 = np.hstack((x_ctr - ws + ws,
                         y_ctr - 0.5 * (hs - 1),
                         x_ctr + 0.5 * (ws - 1),
                         y_ctr + 0.5 * (hs - 1)))
    ### left
    anchors2 = np.hstack((x_ctr - 2 * (ws - 1),
                         y_ctr - 0.5 * (hs - 1),
                         x_ctr + ws - ws,
                         y_ctr + 0.5 * (hs - 1)))
    ### below
    anchors3 = np.hstack((x_ctr - 0.5 * (ws - 1),
                         y_ctr - 0.5* (hs - 1),
                         x_ctr + 0.5 * (ws - 1),
                         y_ctr + hs - hs))
    ### top
    anchors4 = np.hstack((x_ctr - 0.5 * (ws - 1),
                         y_ctr - hs + hs,
                         x_ctr + 0.5 * (ws - 1),
                         y_ctr + 2 * (hs - 1)))
    return np.concatenate((anchors1, anchors2, anchors3, anchors4))

...
def generate_anchors(base_size=16, ratios=[0.5, 1, 2],
    ...
    base_anchor = np.array([1, 1, 2, 2]) - 1   # 4 change by wu
    ratio_anchors = _ratio_enum(base_anchor, ratios)
    anchors = np.vstack([_scale_enum(ratio_anchors[i, :], scales)
                         for i in range(ratio_anchors.shape[0])])
    return anchors.astype(np.float32)
...
def _ratio_enum(anchor, ratios):
    ...
    """
    # ratios: [1, 1 / 2, 2., 1 / 3., 3., 5., 1 / 5.]
    w, h, x_ctr, y_ctr = _whctrs(anchor)
    size = w * h                           #  4
    size_ratios = size / ratios           #[4 8 2 12 4/3 4/5 20]
    ws = np.round(np.sqrt(size_ratios))  # [2, 2.8, 1.4, 3.5,        1.2, 0.89,  4.5]
    hs = np.round(ws * ratios)           # [2. 1.4, 2.8 1.16666667   3.6  4.45   0.9]

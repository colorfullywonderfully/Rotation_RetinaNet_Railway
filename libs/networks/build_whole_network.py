...
from libs.box_utils import anchor_utils, generate_anchors, generate_rotate_anchors, generate_anchor_rail, generate_anchors_catenary
        ...
        with tf.variable_scope('make_anchors'):
            ...
            with tf.name_scope('make_anchors_all_level'):
                for level, base_anchor_size, stride in zip(level_list, cfgs.BASE_ANCHOR_SIZE_LIST, cfgs.ANCHOR_STRIDE):
                    ...
                    # with tf.Session():
                    #
                    #      featuremap_height.eval()
                    #      print ('featuremap_height ',type(featuremap_height))
                    ...
                    if self.method == 'H':
                        tmp_anchors = tf.py_func(generate_anchors.generate_anchors_pre,
                                                 inp=[featuremap_height, featuremap_width, stride,
                                                      np.array(cfgs.ANCHOR_SCALES) * stride, cfgs.ANCHOR_RATIOS, 2.0], #4.0
                                                 Tout=[tf.float32])

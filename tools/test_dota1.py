...
def worker(gpu_id, images, det_net, args, result_queue):
    ...
    with tf.Session(config=config) as sess:
        ...
        for img_path in images:
            ...
            score_res_rotate_ = []
            threshold = {
                         # 'roundabout': 0.1, 'tennis-court': 0.3, 'swimming-pool': 0.1, 'storage-tank': 0.2,
                         ...
                         # 'bridge': 0.0001, 'basketball-court': 0.3, 'baseball-diamond': 0.3, 'H_Bird_Prev_Area': 0.01,
                         #'railway_track': 1
                # 'Tube_split_pinA_normal': 0.1,
                # 'Tube_split_pinA_missing': 0.1,
                # 'Tube_split_pinA_latent_missing': 0.1,
                # 'Tube_split_pinB_normal': 0.1,
                # 'Tube_split_pinB_missing': 0.1,
                # 'Tube_split_pinB_latent_missing': 0.1,
                # 'Tube_pin_normal': 0.1,
                # 'Tube_pin_missing': 0.1,
                # 'Tube_Anti_off_nut-1': 0.1,
                # 'Tube_Anti_off_nut-2': 0.1,
                # 'Tube_puller_bolt_normal': 0.1,
                # 'Tube_puller_bolt_latent_missing': 0.1,
                # 'Anchor_plate_Anti_off_nut-1': 0.1,
                # 'Anchor_plate_Anti_off_nut-2': 0.1,
                # 'Anti_bird_cover_normal': 0.1,
                # 'Anti_bird_cover_abnormal': 0.1,
                # 'Branch': 0.1
                'NutA_1': 0.5,
                'NutA_2': 0.5,
                'NutB_1': 0.5,
                'NutB_2': 0.5,
                'PinA_normal': 0.5,
                'PinA_potential_missing': 0.5,
                'PinA_missing': 0.5,
                'PinB_normal':0.5,
                'PinB_potential_missing': 0.5,
                'PinB_missing': 0.5,
                'Puller_bolt_normal':0.5,
                'Puller_bolt_missing': 0.5
            }
...
def test_dota(det_net, real_test_img_list, args, txt_name):
    ...
    save_path = os.path.join('/home/yunpengwu/RetinaNet_Tensorflow_Rotation-master/tools/test_dota/', cfgs.VERSION)#./test_dota
        ...
        if args.show_box:
            ...
            for i, rbox in enumerate(rboxes):
                command = '%s %.3f %.1f %.1f %.1f %.1f %.1f %.1f %.1f %.1f\n' % (
                                                                                 LABEL_NAME_MAP[res['labels'][i]],
                                                                                 res['scores'][i],
                                                                                 rbox[0], rbox[1], rbox[2], rbox[3],
                                                                                 rbox[4], rbox[5], rbox[6], rbox[7],)
                if res['scores'][i] >0.5:
                    fw = open(res['image_id'].split('/')[-1].split('.')[0], 'a+')
                    fw.write(command+'\n')

                   #write_handle[res['image_id'].split('/')[-1].split('.')[0]].write(command)
            #
            # for sub_class in CLASS_DOTA:
            #     if sub_class == 'back_ground':
            #         continue
...
def parse_args():
    ...
    parser.add_argument('--test_dir', dest='test_dir',
                        ...
                        default='../data/DATA/test/images/', type=str)#/data/DOTA/test/images/
    ...
    parser.add_argument('--show_box', '-s', default=False,  #False
                        action='store_true')

...
def next_batch(dataset_name, batch_size, shortside_len, is_training):
    ...
    valid_dataset= ['ship', 'ICDAR2015', 'pascal', 'coco', 'bdd100k', 'DOTA', 'DOTA800', 'DOTA600', 'HRSC2016', 'RAIL','rail','catanary']
    if dataset_name not in valid_dataset:
        raise ValueError('dataSet name must be in {}'.format(valid_dataset))

    if is_training:
        #pattern = os.path.join('/home/yunpeng/RetinaNet_Tensorflow_Rotation-master/data/tfrecord/', dataset_name + '_train*')
        pattern = os.path.join('/home/yunpengwu/RetinaNet_Tensorflow_Rotation-master/data/io/DOTA/train/tfrecord/RAIL_train.tfrecord')##
    else:
        pattern = os.path.join('/home/yunpengwu/RetinaNet_Tensorflow_Rotation-master/data/io/DOTA/train/tfrecord/RAIL_test.tfrecord')#('../data/tfrecord', dataset_name + '_test*')
